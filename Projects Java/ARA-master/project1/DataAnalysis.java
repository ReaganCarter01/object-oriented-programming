...
public class DataAnalysis{
	...
	public static List sortByAvgDiff(List<AbstractRatingSummary> inList) {
		Collections.sort(inList, new Comparator<AbstractRatingSummary>(){
			public int compare(AbstractRatingSummary rate1, AbstractRatingSummary rate2){
				return rate1.avgScore().compareTo(rate2.avgScore());
			}
		});
		return inList;
    }
	...
	public static List sortByStDevDiff(List<AbstractRatingSummary> inList) {
		Collections.sort(inList, new Comparator<AbstractRatingSummary>(){
			public int compare(AbstractRatingSummary rates1, AbstractRatingSummary rates2){
				return rates1.stDevScore().compareTo(rates2.stDevScore());
			}
		});
		return inList;
	}
